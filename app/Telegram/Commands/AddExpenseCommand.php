...
class AddExpenseCommand extends LoggedInCommand
{
    ...
    public function executeWithUserLoggedIn()
    {
        ...
        $rawCategory = $this->argument('category');
        $categoryName = $rawCategory ? trim($rawCategory, '"\'') : null;
        $categoryId = $categoryName ? $this->findCategoryByName($categoryName)->id : null;
        ...
        Transaction::create([
            ...
            'type' => 'out',
            'value' => $valor,
            'name' => $description, // Usa a descrição limpa
            'description' => $description, // Usa a descrição limpa
